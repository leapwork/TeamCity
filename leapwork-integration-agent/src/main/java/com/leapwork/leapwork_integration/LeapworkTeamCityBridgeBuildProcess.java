...
class LeapworkTeamCityBridgeBuildProcess extends FutureBasedBuildProcess {
	...
	public LeapworkTeamCityBridgeBuildProcess(@NotNull final BuildRunnerContext context) {
		super(context);
	...
	public BuildFinishedStatus call() throws Exception {
		...
		try {
			...
			LinkedHashMap<UUID, String> schedulesIdTitleHashMap = pluginHandler.getSchedulesIdTitleHashMap(mainClient,
					leapworkAccessKey, controllerApiHttpAddress, rawScheduleList, logger, invalidSchedules);
			...
			while (iter.hasNext()) {
				...
				UUID runId = pluginHandler.runSchedule(mainClient, controllerApiHttpAddress, leapworkAccessKey, schId,
						schTitle, logger, run, scheduleVariablesRequestPart);
				if (runId != null) {
					resultsMap.put(runId, run);
					CollectScheduleRunResults(controllerApiHttpAddress, leapworkAccessKey, runId, schTitle, timeDelay,
							isDoneStatusAsSuccess, writePassedKeyframes, run, logger, repeatedNameMapCounter);
				} else
	...
	private static void CollectScheduleRunResults(String controllerApiHttpAddress, String accessKey, UUID runId,
			...
			LeapworkRun resultRun, final BuildProgressLogger logger, HashMap<String, Integer> repeatedNameMapCounter)
			...
			do {
				...
				for (ListIterator<UUID> iter = executedRunItems.listIterator(); iter.hasNext();) {
					...
					RunItem runItem = pluginHandler.getRunItem(client, controllerApiHttpAddress, accessKey, runItemId,
							scheduleName, isDoneStatusAsSuccess, writePassedKeyframes, logger);
					...
					case "Passed":
						...
						if (writePassedKeyframes) 
							{logger.message(pluginHandler.successMessage.get(runItem.getCaseName()));}
						logger.message(TeamcityServiceMessagesFormatter.getInstance()
								.testFinishedMessage(runItem.getCaseName(), Double.toString(runItem.getElapsedTime())));
						resultRun.incPassed();
